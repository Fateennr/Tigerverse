...
const apiBase = process.env.NEXT_PUBLIC_BACKEND_URI;
...
export default function Page() {
  ...
  const [opponentsOptions, setOpponentsOptions] = useState([]);

  // Fetch filter dropdown data once
  useEffect(() => {
    async function fetchOptions() {
      try {
        const [spanRes, venueRes, oppRes] = await Promise.all([
          fetch(`${apiBase}/services/span`),
          fetch(`${apiBase}/services/venues`),
          fetch(`${apiBase}/services/opponents`),
        ]);
  
        if (spanRes.ok) {
          const spanData = await spanRes.json();              // ← await JSON
          const options = spanData.map((item, idx) => ({
            id: idx + 1,
            value: item.Span,         // or whatever your API key actually is
            label: item.Span,
          }));
          setSpans(options);
        }
        ...
        if (venueRes.ok) {
          const venueData = await venueRes.json();            // ← await JSON
          const options = venueData.map((item, idx) => ({
            id: idx + 1,
            value: item.Venue,       // use the correct field name
            label: item.Venue,
          }));
          setVenues(options);
        }
        ...
        if (oppRes.ok) {
          const oppData = await oppRes.json();                // ← await JSON
          const options = oppData.map((item, idx) => ({
            id: idx + 1,
            value: item.Opponent,    // use the correct field name
            label: item.Opponent,
          }));
          setOpponentsOptions(options);
        }
      } catch (err) {
  ...
  // ——— Static sample data (still here to power the UI) ———
  // Replace with dynamic data from your API as needed
  const generalStats = [
    {
          ...
          <div className="absolute inset-0 bg-[#004a37]/95 via-[#005a42]/80 to-transparent flex items-center">
            <div className="px-6 md:px-10 max-w-2xl">
              ...
              <div className="flex flex-wrap gap-3">
                {["general","batting","bowling"].map((tab) => (
                  <button
                    key={tab}
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium backdrop-blur-sm transition-all ${
                      activeTab === tab
                        ? {
                            general: "bg-[#006a4e] text-white",
                            batting: "bg-red-700 text-white",
                            bowling: "bg-yellow-700 text-white"
                          }[tab]
                        : "bg-white/20 hover:bg-white/30 text-white"
                    }`}
                  >
                    {tab === "general" ? "General Stats" : tab === "batting" ? "Batting Records" : "Bowling Records"}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
            <div className="bg-gradient-to-br from-[#006a4e]/90 to-black/90 rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto backdrop-blur-sm">
              <div className="p-6">
                ...
                {/* Detailed modal content omitted for brevity */}
              </div>
            </div>
          </div>
        )}
